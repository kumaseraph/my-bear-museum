...
def main(config):
    ...
    # 建立配送項目：固定角色（4格漫 + 單格插圖）+ 多元內容（5張）
    log("\n--- 步驟 3: 準備配送項目 ---")
    items = []

    # 固定角色：4格漫
    metadata_4grid = prepare_meow_metadata("4grid", styles[0])
    filename_4grid = meow_filename("4grid", today, 1)
    items.append({
        "metadata": metadata_4grid,
        "filename": filename_4grid,
    })
    log(f"  4格漫: {filename_4grid} ({styles[0]})")

    # 固定角色：單格插圖
    metadata_single = prepare_meow_metadata("single", styles[1])
    filename_single = meow_filename("single", today, 1)
    items.append({
        "metadata": metadata_single,
        "filename": filename_single,
    })
    log(f"  單格: {filename_single} ({styles[1]})")

    # 多元內容：5張
    breeds = ["英國短毛貓", "布偶貓", "黑貓", "三花貓", "橘貓"]
    for i in range(5):
        breed = breeds[i] if i < len(breeds) else get_random_breed()
        theme = get_random_theme()
        metadata = prepare_cat_metadata(breed, styles[2 + i], theme)
        filename = cat_filename(today, i + 1, styles[2 + i])
        items.append({
            "metadata": metadata,
            "filename": filename,
        })
        log(f"  多元{i+1}: {filename} ({breed}/{styles[2+i]})")
    ...
    log("\n--- 步驟 6: Git Commit ---")
    run_cmd(f"cd {PROJECT_DIR} && git add cats/cats.json cats/ vocabulary/meow-style-rotation.json cats/daily-meow-delivery.py")
